    ...
    return (
        <div className="flex-1 p-4 min-w-0">
            ...
            <div className="flex items-center justify-between mb-3">
                ...
                <span className="text-lg font-medium text-slate-600 tracking-wide">
            ...
            <div className="grid grid-cols-7 mb-1">
                {DAY_NAMES.map(d => (
                    <div key={d} className="text-center text-[12px] font-bold uppercase tracking-widest text-slate-600 py-1">
                        {d}
                    </div>
                ))}
            </div>
                ...
                {cells.map((date, i) => {
                    ...
                    return (
                        <button
                            ...
                            className={[
                                "flex items-center justify-center transition-all select-none aspect-square w-full h-16 text-sm relative",
                                // range coloring
                                inRange  ? "bg-violet-300 text-white rounded-none"   : "",
                                isStart  ? "bg-violet-600 text-white font-medium " + (hi && !sameDay(lo, hi) ? "rounded-l-lg rounded-r-none" : "rounded-lg") : "",
                                isEnd    ? "bg-violet-600 text-white font-medium rounded-r-lg rounded-l-none" : "",
                                !isStart && !isEnd && !inRange ? "rounded-lg" : "",
                                // default states
                                !isStart && !isEnd && !inRange ? (isToday ? "text-violet-400" : "text-slate-400 hover:bg-slate-700 hover:text-white") : "",
                            ].join(" ")}
                        >
                            <span className={["relative", isToday && !isStart && !isEnd ? "after:absolute after:bottom-[-4px] after:left-1/2 after:-translate-x-1/2 after:w-1 after:h-1 after:rounded-full after:bg-violet-500" : ""].join(" ")}>
                                {date.getDate()}
                            </span>
                        </button>
                    );
